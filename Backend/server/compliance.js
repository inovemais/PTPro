  ...
  router
    ...
    .post(Users.autorize([scopes.Client]), function (req, res, next) {
      ...
      Workouts.WorkoutService.createLog(body)
        .then((result) => {
          // Se o status for "missed", emitir evento Socket.IO para o trainer
          if (result && result.status === "missed" && io && result.trainerId) {
            io.to(String(result.trainerId)).emit("client:missed_workout", {
  ...
  router
    ...
    .put(Users.autorize([scopes.Client]), function (req, res, next) {
      ...
      Workouts.WorkoutService.updateLog(logId, body)
        .then((log) => {
          // Se o status for atualizado para "missed", emitir evento Socket.IO
          if (log && log.status === "missed" && io && log.trainerId) {
